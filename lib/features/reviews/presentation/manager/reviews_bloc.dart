...
class ReviewsBloc extends Bloc<ReviewsEvent, ReviewsState> {
  ...
  ReviewsBloc({required RecipeRepository repo, required int recipeId})
      : _repo = repo,
  ...
  Future load(ReviewsLoading event, Emitter emit) async {
    ...
    final recipe = await _repo.fetchReviewRecipe(event.recipeId);
    final comments = await _repo.fetchComments(event.recipeId);
    emit(ReviewsState(
      recipeModel: recipe,
